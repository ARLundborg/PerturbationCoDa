...
df["School Income Quantile"] = (
    pd.qcut(df["School Income Estimate"], 3, ["low", "medium", "high"])
...
for column in Y_columns:
    ...
    for covariates in ["no", "yes"]:
        if covariates == "yes":
            X = np.c_[
                pd.get_dummies(df["School Income Quantile"]),
                pd.get_dummies(df["School Type"]),
                df[
                    [
                        "Student Attendance Rate",
                        "Percent of Students Chronically Absent",
        ...
        naive_est = res["estimate"]

        res = pert.cdi_gini(
            Y, Z, Y_regression, L_regression, X, folds=folds, random_state=rng
        )
        ...
        print(
            "Y = {}, covariates = {} (n = {})".format(
                column, covariates, len(Y)
            )
        )
        print(
            "CDI_Gini        {:+.3f} -> {:+.3f} in Y per 0.1 of the "
            "index".format(res["estimate"], 0.1 * res["estimate"])
        )
        print(
            "naive_diversity {:+.3f} -> {:+.3f} in Y per 0.1 of the "
            "index".format(naive_est, 0.1 * naive_est)
        )
