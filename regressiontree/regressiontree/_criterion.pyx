...
cdef class Criterion:
    ...
    cdef double proxy_impurity_improvement(self) nogil:
        ...
        cdef double mrss
        self.children_impurity(&impurity_left, &impurity_right, &mrss)
...
cdef class RegressionCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs, SIZE_t n_samples):
        ...
        self.res_sq_sum_total = np.zeros(n_outputs, dtype=np.float64)
        self.res_sq_sum_left = np.zeros(n_outputs, dtype=np.float64)
        self.res_sq_sum_right = np.zeros(n_outputs, dtype=np.float64)
    ...
    cdef int init(
        ...
        self.y = y
        self.X = X
        self.curr_feature = curr_feature
        self.sample_weight = sample_weight
        self.samples = samples
        self.start = start
        self.end = end
        ...
        for p in range(start, end):
            ...
            for k in range(self.n_outputs):
                ...
                self.res_sq_sum_total[k] += w_y_ik * y_ik
...
cdef class MSE(RegressionCriterion):
    ...
    cdef double proxy_impurity_improvement(self) nogil:
        """Compute a exact impurity reduction.
...
cdef class LocLin(RegressionCriterion):
    """Local linear impurity criterion.
    ...
    cdef double node_impurity(self) nogil:
        ...
        cdef double res_sq_sum = 0.0
        cdef double sum_tot = 0.0
        cdef double impurity = 0.0

        cdef double covXY_k = 0.0
        cdef double covXX = 0.0
        cdef double muY = 0.0
        cdef double muX = 0.0
        cdef double theta
        ...
        # Compute sum of squares impurity on full node
        for k in range(self.n_outputs):            
            # Compute means
            for p in range(start, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature]
                y_ik = self.y[i, k]
                if sample_weight is not None:
                    w = sample_weight[i]
                muX += w * x_k/(end - start)
                muY += w * y_ik/(end - start)            
            # Compute regression coefficient
            for p in range(start, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature] - muX
                y_ik = self.y[i, k] - muY
                if sample_weight is not None:
                    w = sample_weight[i]
                covXX += w * x_k * x_k
            ...
            if covXX == 0.0:
                ...
                theta = covXY_k/covXX
            # Compute res_sq_sum_left and sum_left
            for p in range(start, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature]
                y_ik = self.y[i, k]
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum += w * (y_ik - theta*x_k)**2
                sum_tot += w * (y_ik - theta*x_k)
            # Compute impurity
            impurity += res_sq_sum - (sum_tot**2) / (end - start)
            # Reset variables
            covXY_k = 0.0
            covXX = 0.0
            muY = 0.0
    ...
    cdef double proxy_impurity_improvement(self) nogil:
        """Compute exact impurity improvement

        """

        cdef SIZE_t k
        cdef double impurity_tot = 0.0
        cdef double impurity_left = 0.0
        cdef double impurity_right = 0.0
        cdef double impurity_improve = 0.0
        
        impurity_tot = self.node_impurity()

        for k in range(self.n_outputs):
            # Compute impurity left
            if self.weighted_n_left > 0.0:
                impurity_left += self.res_sq_sum_left[k] - (self.sum_left[k]**2) / self.weighted_n_left
            # Compute impurity right
            if self.weighted_n_right > 0.0:
                impurity_right += self.res_sq_sum_right[k] - (self.sum_right[k]**2) / self.weighted_n_right

        impurity_left /= self.n_outputs
        impurity_right /= self.n_outputs

        # Compute difference
        impurity_improve = impurity_tot
        impurity_improve -= impurity_left * self.weighted_n_left / self.weighted_n_node_samples
        impurity_improve -= impurity_right * self.weighted_n_right / self.weighted_n_node_samples
        impurity_improve *= (self.weighted_n_node_samples / self.weighted_n_samples)
        
        return impurity_improve
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        cdef double res_sq_sum_left = 0.0
        cdef double res_sq_sum_right = 0.0
        cdef double sum_left = 0.0
        cdef double sum_right = 0.0
        cdef double sum_total = 0.0
        ...
        # Compute sum of squares impurity on each child
        # Left child:
        for k in range(self.n_outputs):            
            # Compute means
            for p in range(start, pos):
            ...
            for p in range(start, pos):
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum_left += w * (y_ik - theta*x_k)**2
                sum_left += w * (y_ik - theta*x_k)
                sum_total += w * y_ik
            # Reset variables
            covXY_k = 0.0
            covXX = 0.0
            ...
            muX = 0.0
            # Update sum_left
            self.res_sq_sum_left[k] = res_sq_sum_left
            self.sum_left[k] = sum_left
        ...
        for k in range(self.n_outputs):
            ...
            if covXX == 0.0:
                ...
                theta = covXY_k/covXX
            # Compute res_sq_sum_right and sum_right
            for p in range(pos, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature]
                y_ik = self.y[i, k]
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum_right += w * (y_ik - theta*x_k)**2
                sum_right += w * (y_ik - theta*x_k)
                sum_total += w * y_ik
            # Reset variables
            covXY_k = 0.0
            covXX = 0.0
            ...
            muX = 0.0
            # Update sum_right
            self.res_sq_sum_right[k] = res_sq_sum_right
            self.sum_right[k] = sum_right
            # Update sum_total
            self.res_sq_sum_total[k] = res_sq_sum_right + res_sq_sum_left
            self.sum_total[k] = sum_total
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            # Compute impurity left
            if self.weighted_n_left > 0.0:
                impurity_left[0] += self.res_sq_sum_left[k] - (self.sum_left[k]**2) / self.weighted_n_left
            # Compute impurity right
            if self.weighted_n_right > 0.0:
                impurity_right[0] += self.res_sq_sum_right[k] - (self.sum_right[k]**2) / self.weighted_n_right

        impurity_left[0] /= self.n_outputs
        impurity_right[0] /= self.n_outputs


cdef class LocQuad(RegressionCriterion):
    """Local quadratic impurity criterion.
    ...
    cdef double node_impurity(self) nogil:
        ...
        cdef double covXY_k = 0.0
        cdef double covX2Y_k = 0.0        
        cdef double covXX = 0.0
        cdef double covX2X2 = 0.0
        cdef double covXX2 = 0.0
        cdef double muY = 0.0
        cdef double muX = 0.0
        cdef double muX2 = 0.0
        cdef double det
        cdef double theta0
        cdef double theta1
        ...
        for k in range(self.n_outputs):            
            ...
            for p in range(start, end):
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                muX += w * x_k/(end - start)
                muX2 += w * x2_k/(end - start)
                muY += w * y_ik/(end - start)            
            # Compute regression coefficients
            for p in range(start, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature] - muX
                x2_k = self.X[i, self.curr_feature] ** 2 - muX2
                y_ik = self.y[i, k] - muY
                if sample_weight is not None:
                    w = sample_weight[i]
                covXX += w * x_k * x_k
                ...
                covXX2 += w * x_k * x2_k                
                covXY_k += w * x_k * y_ik
                covX2Y_k += w * x2_k * y_ik
                det = (covXX - covX2X2)*covXX2
            if det == 0.0:
                theta0 = 0
                theta1 = 0                
            else:
                theta0 = (covX2X2*covXY_k - covXX2 * covX2Y_k)/det
                theta1 = (covXX*covX2Y_k - covXX2 * covXY_k)/det                
            # Compute res_sq_sum_left and sum_left
            for p in range(start, end):
                i = samples[p]
                x_k = self.X[i, self.curr_feature]
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum += w * (y_ik - theta0*x_k - theta1*x2_k)**2
                sum_tot += w * (y_ik - theta0*x_k - theta1*x2_k)
            # Compute impurity
            impurity += res_sq_sum - (sum_tot**2) / (end - start)
            # Reset variables
            covXY_k = 0.0
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        for k in range(self.n_outputs):
            ...
            for p in range(start, pos):
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum_left += w * (y_ik - theta0*x_k - theta1*x2_k)**2
                sum_left += w * (y_ik - theta0*x_k - theta1*x2_k)
                sum_total += w * y_ik
            # Reset variables
            covXY_k = 0.0
            covX2Y_k = 0.0        
        ...
        for k in range(self.n_outputs):
            ...
            for p in range(pos, end):
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum_right += w * (y_ik - theta0*x_k - theta1*x2_k)**2
                sum_right += w * (y_ik - theta0*x_k - theta1*x2_k)
                sum_total += w * y_ik
            # Reset variables
            covXY_k = 0.0
            covX2Y_k = 0.0        
...
cdef class LocLin_firstonly(RegressionCriterion):
    """Local linear impurity criterion on first feature, MSE criterion otherwise.
    ...
    cdef int init(
        ...
        cdef double res_sq_sum = 0.0
        cdef double weighted_sample_size = 0.0
        ...
        # Compute weighted sample size
        for p in range(start, end):
            if sample_weight is not None:
                w = sample_weight[i]
            weighted_sample_size += w
        self.weighted_n_node_samples = weighted_sample_size
        ...
        for k in range(self.n_outputs):
            if self.curr_feature == 0:
                ...
                for p in range(start, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    muX += w * x_k / weighted_sample_size
                    muY += w * y_ik / weighted_sample_size
                # Compute regression coefficient
                for p in range(start, end):
                    i = samples[p]
                    x_k = self.X[i, self.curr_feature] - muX
                ...
                for p in range(start, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    res_sq_sum += w * (y_ik - muY - theta*x_k)**2
                ...
                for p in range(start, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    muY += w * y_ik / weighted_sample_size
                # Compute res_sq_sum and sum
                for p in range(start, end):
                    i = samples[p]
                    y_ik = self.y[i, k]
                    if sample_weight is not None:
                        w = sample_weight[i]
                    res_sq_sum += w * (y_ik - muY) ** 2
                    self.sum_total[k] += w * y_ik
            # Compute sum of squares
            self.res_sq_sum_total[k] = res_sq_sum * 1.0
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            # Compute total impurity
            impurity += self.res_sq_sum_total[k]
            
        impurity /= (self.n_outputs * self.weighted_n_node_samples)
    ...
    cdef double impurity_improvement(self, double impurity_parent,
            ...
            # Recompute parent impurity (for some reason impurity_parent is different)
            for k in range(self.n_outputs):
                # Compute total impurity
                impurity_tot += self.res_sq_sum_total[k]
            impurity_tot /= self.n_outputs
            
            return ((impurity_tot
                     - self.weighted_n_right * impurity_right
                     - self.weighted_n_left * impurity_left) / self.weighted_n_samples)
    ...
    cdef double proxy_impurity_improvement(self) nogil:
        ...
        for k in range(self.n_outputs):
            # Compute impurity left
            if self.weighted_n_left > 0.0:
                impurity_left += self.res_sq_sum_left[k]
            # Compute impurity right
            if self.weighted_n_right > 0.0:
                impurity_right += self.res_sq_sum_right[k]
            # Compute total impurity
            impurity_tot += self.res_sq_sum_total[k]
        ...
        # Compute difference
        impurity_improve = impurity_tot
        impurity_improve -= impurity_left
        impurity_improve -= impurity_right
        impurity_improve /= self.weighted_n_samples
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        cdef double res_sq_sum_left = 0.0
        cdef double res_sq_sum_right = 0.0
        cdef double res_sq_sum_total = 0.0
        ...
        # Compute sum of squares impurity for total node
        for k in range(self.n_outputs):
            self.sum_total[k] = 0
            if self.curr_feature == 0:
                # Compute means
                for p in range(start, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    muX += w * x_k / self.weighted_n_node_samples
                    muY += w * y_ik / self.weighted_n_node_samples
                # Compute regression coefficient
                for p in range(start, end):
                    i = samples[p]
                    x_k = self.X[i, self.curr_feature] - muX
                ...
                if covXX == 0.0:
                    ...
                    theta = covXY_k/covXX
                # Compute res_sq_sum_total and sum_total
                for p in range(start, end):
                    i = samples[p]
                    x_k = self.X[i, self.curr_feature] - muX
                    y_ik = self.y[i, k]
                    if sample_weight is not None:
                        w = sample_weight[i]
                    res_sq_sum_total += w * (y_ik - muY - theta*x_k)**2
                    self.sum_total[k] += w * y_ik
            else:
                # Compute mean
                for p in range(start, end):
            ...
            muX = 0.0
            # Update sum_total
            self.res_sq_sum_total[k] = res_sq_sum_total * 1.0
            # Reset variables
            res_sq_sum_total = 0.0
        ...
        for k in range(self.n_outputs):
            if self.curr_feature == 0:
                ...
                for p in range(start, pos):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    muX += w * x_k / self.weighted_n_left
                    muY += w * y_ik / self.weighted_n_left
                # Compute regression coefficient
                for p in range(start, pos):
                    i = samples[p]
                    x_k = self.X[i, self.curr_feature] - muX
                ...
                for p in range(start, pos):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    res_sq_sum_left += w * (y_ik - muY - theta*x_k)**2
            else:
                # Compute means
                for p in range(start, pos):
                    i = samples[p]
            ...
            muX = 0.0
            # Update sum_left
            self.res_sq_sum_left[k] = res_sq_sum_left * 1.0
            # Reset variables
            res_sq_sum_left = 0.0
        ...
        for k in range(self.n_outputs):
            if self.curr_feature == 0:
                ...
                for p in range(pos, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    muX += w * x_k / self.weighted_n_right
                    muY += w * y_ik / self.weighted_n_right
                # Compute regression coefficient
                for p in range(pos, end):
                    i = samples[p]
                    x_k = self.X[i, self.curr_feature] - muX
                ...
                for p in range(pos, end):
                    ...
                    if sample_weight is not None:
                        w = sample_weight[i]
                    res_sq_sum_right += w * (y_ik - muY - theta*x_k)**2
            else:
                # Compute intercept
                for p in range(pos, end):
                    i = samples[p]
            ...
            muX = 0.0
            # Update sum_right
            self.res_sq_sum_right[k] = res_sq_sum_right * 1.0
            # Reset variables
            res_sq_sum_right = 0.0
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            # Compute impurity left
            if self.weighted_n_left > 0.0:
                impurity_left[0] += self.res_sq_sum_left[k]
            # Compute impurity right
            if self.weighted_n_right > 0.0:
                impurity_right[0] += self.res_sq_sum_right[k]
            # Compute MRSS
            if self.weighted_n_node_samples > 0.0:
                mrss[0] += self.res_sq_sum_total[k]

        impurity_left[0] /= (self.n_outputs * self.weighted_n_left)
        impurity_right[0] /= (self.n_outputs * self.weighted_n_right)

        # Computes the MRSS at the node level not the child level!
        mrss[0] /= (self.weighted_n_node_samples * self.n_outputs)

cdef class LocLin_first(RegressionCriterion):
    """Local linear impurity criterion on first feature.
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        for k in range(self.n_outputs):
            ...
            for p in range(start, end):
                ...
                if sample_weight is not None:
                    w = sample_weight[i]
                res_sq_sum_total += w * (y_ik - theta*x_k)**2
                sum_total += w * y_ik
        ...
        for k in range(self.n_outputs):
            ...
            self.sum_right[k] = sum_right
            # Update sum_total
            # self.res_sq_sum_total[k] = sq_sum_total
            # self.sum_total[k] = sum_total
            # Reset variables
            res_sq_sum_right = 0.0
            sum_right = 0.0
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            ...
            # Compute MRSS
            if self.weighted_n_node_samples > 0.0:
                mrss[0] += self.res_sq_sum_total[k] - (self.sum_total[k]**2) / self.weighted_n_node_samples
                # mrss[0] += impurity_left[0] + impurity_right[0]
...
cdef class LocLin_grad(RegressionCriterion):
    """Local linear impurity criterion on first feature, MSE criterion otherwise.
